...
export default function SOnePage() {
  ...
  async function fetchPrestamos() {
    setLoading(true);
    try {
      const { data, error } = await supabase.from('loans').select('*').in('status', ['accepted', 'overdue']).not('disbursed_at', 'is', null);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h1 className="text-3xl font-bold tracking-tight text-primary uppercase">Módulo S1</h1>
        <div className="flex items-center space-x-3">
          ...
          <Badge variant="outline" className="px-4 py-2 text-red-400 border-red-400/20 bg-red-400/5 font-bold">{filteredPrestamos.length} VENCIDOS</Badge>
        </div>
      </div>
            ...
            <Card key={prestamo.id} className="bg-card border-none shadow-xl border-l-4 border-l-red-500 overflow-hidden">
              <div className="flex flex-col md:flex-row md:items-center justify-between p-6 gap-6">
                <div className="flex items-center space-x-5">
                  <div className="h-16 w-16 rounded-2xl bg-red-500/10 flex items-center justify-center text-red-500 shrink-0 overflow-hidden">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 mt-2 text-sm">
                      <span className="flex items-center font-bold text-red-400"><DollarSign className="h-4 w-4 mr-0.5" />{formatAmount(prestamo.amount)}</span>
                      <span className="flex items-center text-red-500 font-bold"><CalendarX className="h-4 w-4 mr-1.5" />Venció: {formatDateDisplay(dueDate.toISOString())}</span>
                    </div>
                  </div>
                </div>
